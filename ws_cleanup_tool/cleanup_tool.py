...
def parse_config():
    ...
    if maybe_config_file:                             # Covers no conf file or only conf file
        ...
        parser.add_argument('-k', '--orgToken', help="WS Organization Key", dest='ws_org_token', required=True)
        ...
        parser.add_argument('-g', '--analyzedProjectTag', help="Allows only analyze whether to clean up when a project contains the specific K:V tag", dest='analyzed_project_tag')
        parser.add_argument('-r', '--daysToKeep', help="Number of days to keep in FilterProjectsByUpdateTime or number of copies in FilterProjectsByLastCreatedCopies", dest='days_to_keep', type=int, default=5)
